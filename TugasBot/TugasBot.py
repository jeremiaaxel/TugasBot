...
class TugasBot(TugasBotInterface):
    ...
    def reset_messages(self) -> TugasBot:
        self.message = ''
    ...
    def set_message(self, message: discord.Message) -> TugasBot:
        self.message = message
        self.parse_message(message.content)
        return self

    def set_bot(self, bot) -> TugasBot:
        self.bot = bot
        return self
    ...
    def add_tugas(self):
        ...
        try:
            data = Parser.extract_tugas(
                self.message.content.replace(f'{cfg.message_prefix}add', ""),
                cfg.url_tugas)
    ...
    '''
        ...
        if cfg.schedule_sheet:
            ...
            try:
                sheet = spreadsheet.worksheet(cfg.schedule_sheet +
                                              str(angkatan))
    ...
    def print_schedule(self, schedule):
        ...
        for day, daily_schedule in compacted_schedule.items():
            ...
            for hour, course in daily_schedule.items():
                curr_hour = hour.replace(" ",
                                         "").split(cfg.schedule_time_separator)
        ...
        try:
            ...
            for day, daily_schedule in schedule.items():
                if day_int_now == Parser.DAY_INT_DICT[day.lower()]:
                    for hour, course in daily_schedule.items():
                        start_hour, end_hour = hour.replace(" ", "").split(
                            cfg.schedule_time_separator)

                        # cek jam saat ini di antara jam di jadwal dan course beda sama sebelumnya

                        if (course != self.get_current_course()
                                and hour_now > start_hour
                                and hour_now < end_hour):
    ...
    def process(self):
        command = self.get_main_command()
        if command == f'test':
            return "Test"

        elif command == f'resend':
            return self.message.content

        # REQUEST spreadsheet
        elif command == f'sheet':
            return self.request_spreadsheet()

        # REQUEST tugas
        elif command == f'tugas':
            return self.request_tugas()

        # REQUEST add tugas
        elif command == f'add':
            return self.add_tugas()

        # REQUEST ujian
        elif command == f'ujian':
            return self.request_ujian()
        ...
        # REQUEST schedule
        elif command == f'schedule':
            return self.request_schedule()
